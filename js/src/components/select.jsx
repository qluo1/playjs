...
class MyOption extends Component {
  render () {
    console.log(`value: ${this.props.value}  ${this.props.state} ${this.props.value ==
        this.props.state} `)

    if (this.props.state === this.props.value) {
      return (
        <button
          className='btn-primary btn-lg active'
    ...
    return (
      <button className='btn disable' onClick={this.props.select}>
        {this.props.value}
      </button>
    ) // react div element, via JSX
  }
